...
import { FaUniversity, FaBriefcase, FaCode, FaCuttlefish, FaPython } from 'react-icons/fa';
    ...
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
      {items.map((item, index) => (
        <a
          ...
          className="bg-white border border-gray-200 rounded-xl shadow-md hover:shadow-xl transition transform hover:scale-105 p-6 flex gap-4 items-start"
        >
          <img
            src={item.logo}
            alt={item.title}
          ...
          <div>
            ...
            <p className="text-sm w-8/12 text-blue-500 break-words">{item.link}</p>
          </div>
        </a>
      ))}
    </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-6 md:px-16">
      <div className="max-w-7xl mx-auto">
        ...
        <Section title="C Programming" items={resources.c} icon={<FaCuttlefish size={28} />} />
        <Section title="C++ Programming" items={resources.cpp} icon={<SiCplusplus size={28} />} />
        <Section title="Python Programming" items={resources.python} icon={<FaPython size={28} />} />
        <Section title="Codeforces Resources" items={resources.codeforces} icon={<FaCode size={28} />} />
        <Section title="ICPC Preparation" items={resources.icpc} icon={<FaUniversity size={28} />} />
        <Section title="Job Interview Prep" items={resources.job} icon={<FaBriefcase size={28} />} />
      </div>
    </div>
  );
};
